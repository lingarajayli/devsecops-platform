# Incident #022: High Memory Usage
...
Application memory usage is high and may lead to OOMKilled, eviction, or node pressure.
...
flowchart TD
    ...
    A --> B["Meaning<br/>Application memory usage is high and may lead to OOMKilled, eviction, or node pressure"]

    B --> C["Common Causes<br/>Memory leak<br/>Cache growth<br/>Large payloads<br/>Bad JVM heap<br/>Traffic spike"]

    C --> D["Investigation<br/>Check memory dashboard<br/>kubectl top pods<br/>Check OOM events<br/>Check previous logs<br/>Check heap/runtime"]

    D --> E["Remediation<br/>Fix leak, tune runtime memory, right-size limits, reduce workload size, or roll back."]
...
Cache grew without eviction and memory usage kept increasing until near the limit.

---

## Remediation

Fix leak, tune runtime memory, right-size limits, reduce workload size, or roll back.
...
`High Memory Usage` means application memory usage is high and may lead to OOMKilled, eviction, or node pressure.
...
Today I documented production-style incident #022: High Memory Usage.
