# Incident #025: Application Latency Spike
...
Application response time suddenly increased and user experience degraded.
...
```mermaid
flowchart TD
    A["Application Latency Spike"]

    A --> B["Meaning<br/>Application response time suddenly increased and user experience degraded"]

    B --> C["Common Causes<br/>Traffic spike<br/>DB slow query<br/>CPU throttling<br/>Memory pressure<br/>Downstream dependency slow"]

    C --> D["Investigation<br/>Check latency dashboard<br/>Check error rate<br/>Check saturation<br/>Check traces<br/>Check DB metrics"]

    D --> E["Remediation<br/>Scale, optimize bottleneck, fix dependency, tune resources, or roll back the bad release."]
...
A new query caused database latency to spike and increased API response time.

---

## Remediation

Scale, optimize bottleneck, fix dependency, tune resources, or roll back the bad release.
...
`Application Latency Spike` means application response time suddenly increased and user experience degraded.
...
Today I documented production-style incident #025: Application Latency Spike.
