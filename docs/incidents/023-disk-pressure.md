# Incident #023: Kubernetes Node Disk Pressure
...
A node is running low on disk, and Kubernetes reports DiskPressure.
...
```mermaid
flowchart TD
    A["Kubernetes Node Disk Pressure"]

    A --> B["Meaning<br/>A node is running low on disk, and Kubernetes reports DiskPressure"]

    B --> C["Common Causes<br/>Container logs<br/>Image cache<br/>Ephemeral storage<br/>Large emptyDir<br/>No log rotation"]

    C --> D["Investigation<br/>kubectl describe node<br/>Check DiskPressure condition<br/>df -h<br/>du large paths<br/>Check logs"]

    D --> E["Remediation<br/>Clean disk safely, configure log rotation, set ephemeral storage limits, or add node capacity."]
...
Container logs filled `/var/log`, causing kubelet to report DiskPressure.

---

## Remediation

Clean disk safely, configure log rotation, set ephemeral storage limits, or add node capacity.
...
`Kubernetes Node Disk Pressure` means a node is running low on disk, and Kubernetes reports DiskPressure.
...
Today I documented production-style incident #023: Kubernetes Node Disk Pressure.
