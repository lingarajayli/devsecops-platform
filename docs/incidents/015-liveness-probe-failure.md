# Incident #015: Kubernetes Liveness Probe Failure
...
Kubernetes believes the container is unhealthy and restarts it repeatedly.
...
```mermaid
flowchart TD
    A["Kubernetes Liveness Probe Failure"]

    A --> B["Meaning<br/>Kubernetes believes the container is unhealthy and restarts it repeatedly"]

    B --> C["Common Causes<br/>Wrong liveness path<br/>Wrong port<br/>Slow startup<br/>Missing startupProbe<br/>Timeout too low"]

    C --> D["Investigation<br/>kubectl describe pod<br/>Check restart count<br/>Check liveness config<br/>Check previous logs<br/>Curl health endpoint"]

    D --> E["Remediation<br/>Fix liveness configuration, use startupProbe for slow apps, or fix the application hang."]
...
The app needs 90 seconds to start, but liveness begins after 10 seconds and kills it early.

---

## Remediation

Fix liveness configuration, use startupProbe for slow apps, or fix the application hang.
...
`Kubernetes Liveness Probe Failure` means kubernetes believes the container is unhealthy and restarts it repeatedly.
...
Today I documented production-style incident #015: Kubernetes Liveness Probe Failure.
