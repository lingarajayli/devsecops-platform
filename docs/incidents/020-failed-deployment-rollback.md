# Incident #020: Failed Deployment Rollback
...
A deployment failed and rollback did not restore service cleanly.
...
flowchart TD
    ...
    A --> B["Meaning<br/>A deployment failed and rollback did not restore service cleanly"]

    B --> C["Common Causes<br/>Bad previous revision<br/>DB migration incompatible<br/>Config drift<br/>Image missing<br/>Helm rollback issue"]

    C --> D["Investigation<br/>Check rollout history<br/>Check ReplicaSets<br/>Check events<br/>Check app logs<br/>Check DB migrations"]

    D --> E["Remediation<br/>Use safe rollback or forward fix after identifying the incompatible change."]
...
- Bad previous revision
- DB migration incompatible
- Config drift
- Image missing
- Helm rollback issue
- Stateful dependency changed
...
Rollback failed because the new deployment applied a non-backward-compatible database migration.

---

## Remediation

Use safe rollback or forward fix after identifying the incompatible change.
...
`Failed Deployment Rollback` means a deployment failed and rollback did not restore service cleanly.
...
Today I documented production-style incident #020: Failed Deployment Rollback.
