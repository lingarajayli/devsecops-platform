# Incident #024: Prometheus Target Down
...
Prometheus cannot scrape metrics from one or more configured targets.
...
flowchart TD
    ...
    A --> B["Meaning<br/>Prometheus cannot scrape metrics from one or more configured targets"]

    B --> C["Common Causes<br/>Service down<br/>Wrong scrape path<br/>Wrong port<br/>ServiceMonitor label mismatch<br/>NetworkPolicy"]

    C --> D["Investigation<br/>Open Prometheus Targets<br/>Check scrape error<br/>Curl /metrics<br/>Check ServiceMonitor<br/>Check service endpoints"]

    D --> E["Remediation<br/>Fix scrape config, labels, port/path, metrics endpoint, service, or network policy."]
...
ServiceMonitor selector did not match the service labels, so Prometheus did not scrape the target.

---

## Remediation

Fix scrape config, labels, port/path, metrics endpoint, service, or network policy.
...
`Prometheus Target Down` means prometheus cannot scrape metrics from one or more configured targets.
...
Today I documented production-style incident #024: Prometheus Target Down.
